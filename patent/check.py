...
gorulen = []
for ad in ("01-description.pdf", "02-claims.pdf", "03-abstract.pdf"):
...
res = []
for w, h, ic in bbox(D + "/04-figures.pdf"):
...
t = subprocess.run(["pdftotext", "-f", "1", "-l", "1", D + "/01-description.pdf", "-"],
                   capture_output=True, text=True).stdout
o = subprocess.run(["pdftotext", D + "/03-abstract.pdf", "-"], capture_output=True, text=True).stdout
def bas(x, unsur):
...
im = subprocess.run(["pdfimages", "-list", D + "/03-abstract.pdf"], capture_output=True, text=True).stdout
...
MM = 72 / 25.4
for w, h, ic in bbox(D + "/01-description.pdf")[:1]:
    w, h = float(w), float(h)
