...
computed that way in `aero/thrust.py`. The two calculations do not belong in one column.
...
conditional on the buffer assumption and that nothing here replaces it. `aero/closure.py` carries
the loop.
