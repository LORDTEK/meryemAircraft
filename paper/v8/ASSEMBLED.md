...
configuration declines rather than a limit it inherits (Section 5). Precise hovering, ground gusts and
...
**None of the elements is new**, and Section 5 says so. Tail-sitting aircraft are seventy years
...
**It means zero of the three charges as Section 2.1 defines them** — the mass of a dedicated lift
...
configuration satisfies the condition is the subject of Sections 3 to 5; and what such a
...
that structurally lacks it, before Section 5 asks whether one aircraft can hold both.
...
charged to the mass budget once** — Section 5 gives the fairing's sizing.
...
aircraft use about the body's longitudinal axis (Section 5), leaving that axis to the strip.
...
section**, and the two are combined in Section 5.
...
Section 2.1; that comparison is not made, and no result in this paper rests on it. The claim is
...
rather than higher — at a propeller efficiency η_p = 0.85 the same airframe would reach 7.47 to 9.20. Section 7.2 charges all three.

**The two halves are now on the table separately. Section 5 is where they are combined**, and
...
exist in order to change regime, or to take a rotor out of one regime's flow.** The strip of Section 5 is a
control surface, of a different class, and is named below and in Section 5 rather than in the table. The configuration therefore carries:
...
torque**, and this configuration declines that channel by design (Section 5), assigning the axis to an aerodynamic
...
Section 5 claimed that a class of mechanism is absent. A claim of that kind is only as good as
...
assignment, not a demonstrated result (Section 5): the moment it produces is a sizing input to Section 7.1,
...
a statement about what hardware is present, and it is settled by the inventory of Section 5. **The
separate claim that this aircraft can actually perform the regime change is not settled** (Section 5).
...
channel that comparable aircraft use for roll (Section 5). What that refusal costs — in thrust asymmetry, in propulsive efficiency, and in response time set by rotor inertia —
...
it, and the accounting** — which is what Section 5 describe and what Section 7.2 prices.
...
This section prices the arrangement of Section 5 on a declared package; it does not bear on
...
different question, and Section 8 is where the answer is not yet yes.
...
effective ratio of Section 4 already contains the propeller efficiency; it is the currency in
...
**Three things the loop does not scale, and a reader comparing this section with Section 5 should
know which is which.** The tip-frame length, the tip-disc diameter and the strip are not sizing
variables here. They were set on the reference geometry — the 50 kg reference design of Section 5 — and **the control moment arms of Section 5 are therefore reference
values that this closure does not re-derive.** Section 5 describes one aeroplane; this section
...
quantity Section 8 reports as **not predicted reliably** — the moment exists; what is missing is
...
the item Section 8 examines, and the examination does not end well. Nothing in this section
...
that Section 8 tests.

**And these range figures are carried forward as the closed-loop values, not as a ranking.** No
multirotor is sized in this work, so no range comparison is made against one — Section 4 compares
the two families in cruise efficiency and says why it stops there. The comparison against the
other hybrid architectures depends on the sizing contract and belongs to Section 7.4, which is
where it is made.

### 7.2 The ledger

Section 2.1 named three charges that any architecture in this corner pays. Section 7.1 closed a
...
Section 7.1 — in the drag bracket, in the propeller efficiency, in the empty-mass fraction, in the
...
and that is Section 7.4: **the total is the contract, not a property of the aircraft.** For a
...
The zero-lift drag coefficient of Section 7.1 is a build-up with named terms. At the favourable end it is 0.0073 for the clean wetted surface, 0.0015 for the hub and small items,
...
Section 7.3 shows how strongly the term depends on it.
...
of it at the favourable end. **That is Bill 2 on this aircraft, in the terms Section 2.1 defined
it** — and the phrase is "exposed by the vertical-phase layout" rather than "dedicated lift group",
because Section 5 is precisely the claim that there is no dedicated lift group here.

**The tip-frame term is an attribution, not a marginal removal cost.** Section 5 gives the frames
...
**This is a statement about position within the drag bracket at one scale.** Section 7.3 asks a
...
and is not carried as a line.** Section 2.1 quotes a wind-tunnel finding that a simulation
...
Section 7.1's closures run at a cruise propeller efficiency of 0.632 to 0.683, against the 0.80 the
...
**There is no dedicated lift group to charge**, which is the architectural claim of Section 5
...
**Section 2.2 said in advance that this would happen and refused to call it free.** The buffer is
not lift-subsystem mass, so it is not Bill 1 as Section 2.1 defines it — but it is mass carried
...
**construction constants held common across the three architectures** so that Section 7.4 compares
...
rather than as the difference of these two shaft figures, and which Section 8 computes.
...
The items above are inside Section 7.1's numbers. **These are not**, and a reader should not
...
behave as the aircraft changes size.** Section 7.3 asks whether they move together when the size
changes, and Section 7.4 asks what happens to the comparison when the sizing contract changes.

### 7.3 Scale does not lock two of the charges together; the third is not tested

Section 7.2 decomposed the three charges on one aircraft, at one size. **This section asks a
...
of Section 2.1 would be a single cost described three ways, and a ledger in three currencies would be
a ledger in one. Either answer leaves the mechanism claim where it was; that claim rests on the
inventory of Section 5.
...
**This is a different axis from the one Section 7.2 examined.** There, Bill 2's share of the
...
closure (both below). A scale comparison therefore cannot be made from Section 7.1's closures. **It
...
those two reference designs. Taking one end from Section 7.1 and the other from the reference pair would manufacture a scale
change that is really a propeller-efficiency update applied to one end only.

**The quantities used are, with one exception, ones Section 7.1 did not replace.** Disc loading is a sizing rule
Section 7.1 holds. The buffer fraction is an input to its loop. The free-wheeling rotor term is the value Section 7.1 carries at both ends of its bracket at 50 kg,
before the ten percent margin of the adverse end (Section 7.2), and it is computed here at 1 000 kg by
the same method. **The total zero-lift drag, the propeller efficiency, the range and the closed mass
are not used.** The exception is the engine rating inside the Bill 3 ratio, which Section 7.1 did replace; it is
...
heavy, a change of 5 percent. *(Section 7.2's 2.4 to 3.2 is the same ratio at the four closures; their
cruise engines, 3.54 to 5.17 kW, are larger than the light reference design's 2.6 kW, and the engine
rating is a quantity Section 7.1 did replace. This paragraph compares the reference pair only.)* But the engine is sized by cruise, not by disc loading, and **the two
...
less certain — and it is the one Sections 7.1 and 7.2 carry.**
...
**On this configuration Bill 1 appears as the energy buffer**, as Section 7.2 set out, since there is
...
the form of the device that releases Bill 3 from the engine, as Section 2.2 anticipated, and **whether
the two are separable here is not established.**

**What is established is that they are coupled here, and that is Section 2.2's claim rather than a
...
fraction scales, and it is unmeasured; but the airframe is not Bill 1 as Section 2.1 defines it — it
...
Section 2.1 asserts; it is not a verification of separability as a general property, which a single
...
Section 7.4 examines what the choice of sizing contract does to a ranking, on the light closures of
Section 7.1 only.

### 7.4 Rankings belong to contracts

Section 7.3 showed that at least two of the three charges are not locked together, and drew the
...
architectures at each of the four closures of Section 7.1. **The mechanism claim is not a ranking
and is not at stake here**; what is at stake is how the price computed in Sections 7.1 and 7.2 enters
...
claim that those families would use this power system, and the tilting family as Section 2.1 describes
...
- **Drag.** All three share the clean airframe at each end of the drag bracket. This configuration
  carries its exposed frames and free-wheeling rotors, as in Sections 7.1 and 7.2. The lift-plus-cruise
  layout carries the ratio measured in the wind-tunnel campaign quoted in Section 2.1 — maximum
  ...
  stopped and aligned in cruise, which takes an indexing mechanism (Section 5) whose mass is not
...
The claim this paper makes against that family is about mechanism (Section 5), and nothing in
this section bears on it.

#### Section 2.1's prediction, tested

**Section 2.1 predicted that where an arrangement pays one charge heavily in order to escape another,
...
the case above it is the blade family, which Section 7.1 leaves open. Across the sensitivity cases
...
**The comparison is at one size.** Section 7.3's 1 000 kg reference design has no closure, and none of its figures
is used here.
...
Section 7.1 closed the sizing loop on a declared package and said that whether an aircraft can be
built to it is a different question. **This section is where that question is answered, and for the
first item the answer is no.** Section 6 called this section a debt: questions the paper does not
...
**Every closure in Section 7.1 carries a buffer of 3.6 percent of take-off mass.** That figure is an
input, not a result (Sections 7.2 and 7.3). What it implies can be computed. Taken at the electrical bus,
...
that of existing batteries. **The take-off demand of Section 7.1's closures is 3.7 to 4.1 times the
...
Section 7.1's — the same fractions, including an airframe at thirty percent of take-off mass, and the same
wing loading, disc loading and aspect ratio, so the lift-to-drag ratio is carried unchanged and, with the
fuel fraction held, so is the range. **These masses are the Section 7.1 package with one input changed.
...
| Buffer specific power, per kilogram of buffer | Take-off mass | Buffer | Change from Section 7.1 |
|---|---:|---:|---:|
| As Section 7.1 implies — 5.5 to 6.1 kW kg⁻¹ | 52.3 to 57.5 kg | 3.6 % | — |
...
percent of take-off mass rather than 3.6. If Section 7.1's take-off masses are retained instead, the
...
**This is where the coupling Section 7.3 found is paid.** The buffer is the conversion the escape
condition permits — kilowatts of hover peak paid in kilograms of store. Section 7.2's ledger records that
conversion at the assumed store; at a measured specific power it costs thirteen to fifteen percent of
take-off mass instead of 3.6. **The escape from Bill 3 is real in the sense Section 2.2 defined it, and
...
**It reaches every number that describes this aircraft at Section 7.1's masses.** The closed masses of
...
survive as 13 kg carried that far on a store that has been built. The vertical phase that Section 3
reports as sized was sized with this store in it. And Section 7.4's orderings were computed with the
store held common at 3.6 percent; how they would move with a measured store is not computed.

**It does not reach the mechanism claim.** Section 5 count the classes of mechanism that a
tilting architecture needs to change regime and this one does not; that is a statement about hardware,
and a heavier store adds no pivot. **Nor does it reach the cruise-efficiency comparison of Section 4
as a ratio**: effective lift-to-drag ratio combines aerodynamic and propulsive efficiencies and has no
mass in it. As a comparison of aircraft, that section describes the configuration at Section 7.1's
...
- **atmosphere** — analysis; the direction of its effect on the Section 4 comparison has not been computed.
...
The last section returns to the four axes of Section 6 and states what is claimed on each.
...
the other (Section 4). Nothing is claimed against multirotors on vertical capability.
...
carries none of the mechanism classes Section 5 counts: no pivot, no nacelle or rotor-group actuator, no
