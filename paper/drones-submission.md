...
`00-front-matter.md` içindeki Author Contributions bloğu hâlâ **⚠️ TASLAK** işaretli.
...
`paper/build/mkdocx.py` `paper-v7.md`'den okuyup MDPI düzeninde `.docx`
...
|---|---|---|
| `meryemAircraft-MDPI.docx` | `paper/pdf/` | **Manuscript** — MDPI düzeninde, gönderilecek olan |
| `meryemAircraft-MDPI-supp.docx` | `paper/pdf/` | **Supplementary File** — aynı düzende |
| `meryemAircraft-paper.pdf` | `paper/pdf/` | yedek; sistem PDF isterse |
| `meryemAircraft-ek.pdf` | `paper/pdf/` | yedek |
| `sekil01`…`sekil12` | `figures/output/` | **Figures**, ayrı ayrı, yüksek çözünürlüklü |

> **Sürüm:** gönderilecek metin **v7**'dir, v6 değil. Zenodo'da v6 yayımlandı ve
> v7 onu düzeltiyor; `paper/VERSIONS.md` hangi dosyanın nerede yayımlandığını
...
| **Abstract** | `00-front-matter.md` → Abstract bloğu. **214 kelime**. Dergi "about 200 words maximum" diyor; 214 "about 200" içinde sayılır ama teknik ön denetim itiraz ederse kısaltılacak yer, tilt paragrafının ikinci yarısıdır |
...
| **Highlights** | `00-front-matter.md` → Highlights bloğu. İki başlık, her birinde iki madde — derginin istediği biçim |
...
1. **`verify.py` ve `links.py` çalıştır.** 43 sayısal kontrol + 9 yasaklı bayat değer + bağ dokusu (işaretçiler doğru yere çözülüyor mu, tablo/şekil atıfları tutuyor mu). Şu an 0 sapma.
2. **PDF'i aç ve göz gezdir** — özellikle Şekil 2 ve Şekil 9 gibi geniş
   grafiklerin sayfaya sığdığını, tabloların taşmadığını.
3. **Özetin kelime sayısı** — 214. `mkrelease.py` 215'i aşarsa kurulumu durduruyor, yani sessizce büyüyemez.
...
depoda `paper-v7.md` / `paper-v7-supp.md`, yayımlandığı commit `8138b62`,
...
depoda yapılacak hiçbir düzeltme o dosyaya dokunmaz (bkz. `VERSIONS.md`);
düzeltmeler `revision-list.md`'de birikir ve hakem raporu geldiğinde
**v8** olarak çıkar.
...
**1. Kapak mektubunda rakip dergiyi ben adlandırdım.** `drones-cover-letter.txt`
...
`revision-list.md` ona göre yeniden düzenlenir. Kesin olan tek şey:
