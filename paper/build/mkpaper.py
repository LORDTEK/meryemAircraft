...
Cikti: paper/paper.md  ve  paper/pdf/meryemAircraft-paper.pdf
"""
...
BOL  = os.path.join(ROOT, "paper", "sections")
...
def blok(metin, baslik):
    """00-front-matter.md icinden '## baslik' ya da '### baslik' bolumunu ceker."""
...
# ---------------------------------------------------------------- montaj
ON = open(os.path.join(OUT, "00-front-matter.md")).read()
...
# Highlights ile AYNI kusur, bir tur sonra tekrar: dordu de 00-front-matter.md
...
# 00-front-matter.md'ye yazilmis ama HICBIR uretim betigine baglanmamisti --
...
for _ad, _v in (("Supplementary Materials", ek_malz), ("Patents", patent),
    ...
    if not (_v or "").strip():
        sys.exit("00-front-matter.md icinde zorunlu blok bos ya da yok: %s" % _ad)
...
ARKA = "\n\n".join([
    ...
    temizle(open(os.path.join(OUT, "bibliography-en.md")).read()),
])
...
MD = ON_HTML + "\n\n" + govde + "\n\n" + ARKA
turkce_denetle(MD, "paper.md")
open(os.path.join(OUT, "paper.md"), "w").write(
...
YATAY_DIZIN = os.path.join(ROOT, "figures", "output", "landscape")
...
def yatay_sekil_uret(n, yollar, cap):
    ...
    cikti = os.path.join(YATAY_DIZIN, "fig%s-landscape.png" % n)
...
PDF = os.path.join(OUT, "pdf", "meryemAircraft-paper.pdf")
...
asyncio.run(yaz())
print("paper.md  ve  %s yazildi" % PDF)
