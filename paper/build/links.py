...
NEDEN VAR. verify.py SAYILARI denetler ve iyi yapar: 40 kontrol, her biri
...
from version import SURUM                 # sabit "v6" yazıliydi; surum
KAYNAK = os.path.join(MAKALE, "paper-%s.md" % SURUM)   # kayinca BAYAT dosyayi denetliyordu
...
sys.path.insert(0, BURA)
from captions import FIGS                                     # noqa: E402

for tur, desen in (("Table", r"\*\*Table (\d+)\.\*\*"),
                   ("Figure", None)):
    if True:
        # Sekil altyazilari bu belgede degil; tanim captions.py'dedir.
