...
Kaynaklar `cfd/source/` altındadır ve depoya girmiştir. Sayılar
`cfd/data/reference.py`'de, her birinin yanında nereden ve **nasıl**
...
Toplam C_D farkın nerede olduğunu söylemez; `common/cf.py` söylüyor.
...
`common/wall_law.py` sınır tabakası profilini duvar değişkenlerinde
...
akışta aynı ölçümü yapmaktır: `plate/flat_plate.py`. Beklenti önceden
yazılı:
...
`naca/sst_setup.py`, SST'ye özgü duvar koşullarını tek tek değiştiriyor.
...
`plate/flat_plate.py`, sıfır basınç gradyanlı düz levha. Neden gerekliydi:
...
(`veri/nas_figure.py`). Bu, şekli gözle okumaktan (raster
...
doğrudan bunun için var ve `compare.py` denetim başarısızsa çalışmayı
durduruyor.
...
`plate/omega_wall.py`, düz levhada duvar ω koşulunu iki onlu bir aralıkta
...
terimi. `plate/model_discriminate.py` bunu ayırıyor — aynı ağ, aynı şemalar,
...
`plate/model_discriminate.py` — aynı ağ, aynı şemalar, aynı sınır koşulları,
tek fark model:
...
1. **Profil karşılaştırması** (`plate/compare.py`). SA'mız iki
...
`plate/schemes.py`, beş varyant, hepsi kOmegaSST:
...
Bu denetim artık `veri/nas_figure.py` içinde `eksen_denetimi()` olarak
duruyor ve `plate/compare.py` saçılım 0,05'i aşarsa **çalışmayı
...
`veri/nas_figure.py` Şekil 7.4'ten SA modeli için üst yüzeydeki C_f
...
gerilmiş ağda hücre sınırlayıcısı orada ısırabilir. `plate/schemes.py` beş
...
**Dört kat uzun koşu.** `plate/flat_plate.py uzun`, 16 000 adım:
...
**Birinci ölçüm — duyarlılık var ve yönü uyuyor** (`sa_free.py`,
...
**İkinci ölçüm — mekanizma yok** (`nutilda_reached.py`). Hipotez, R=200'de
...
Asıl sebep başka yerdeydi ve tarama tasarımının kendisindeydi. `domain.py`,
...
alan taraması" diye bir şey yoktur ve `domain.py`'nin büyüme oranını
...
ayarları ve türbülans kurulumu ikisine de `naca/setup.py`'den aynı şekilde
geliyor (`setup3d.py`), ayrıştırma da aynı (hierarchical 2 2 1).
...
Yerine `cfd/wing/tip_cap_mesh.py`: **halka + Coons çekirdek**.
...
Değişiklik yalnızca `setup3d.py`'de; 2-B zinciri korundu.
...
- İlk açıklamam "5-tet ayrıştırmam OpenFOAM'ınkiyle eşdeğer değil" idi.
  ...
  — yüz merkezli üçgen yelpazesi + piramit toplamı) `common/mesh_audit.py`
...
sayısına göre ortalamayı o belirliyor. `forces.py`'ye **alan ağırlıklı**
...
Ağ **aynı**: `setup.py` hücum açısını ağı döndürerek değil serbest akış
...
| | değer |
|---|---|
| CFD wing/gövde + uç iskeletleri (0,00431) + büyük göbek (0,00195) | 0,01960 |
...
5. CFD yalnızca **wing/gövdeyi** kapsıyor; uç iskeletleri ve göbekler
...
**Geçiş sonucu bu eğimde de ayakta.** `aero/sensitivity.py` CFD'nin
...
sürüklemesini ayıramıyor · CFD yalnızca wing/gövde; uç iskeletleri ve
...
hacmi negatif değil (OpenFOAM'ın kendi yordamıyla, `mesh_audit.py`).
...
Sebep ağlarda değildi: `limited 0.33` şema düzeltmesi `setup3d.py`'ye
...
Gevşeme artık `setup3d.py`'de, ölçümüyle birlikte (`klasik_simple=True`).
...
diye). Hesap `cfd/wing/gci.py`, Celik ve ark. ASME JFE 130(7), 2008.
...
türetilmiş alan (`cfd/common/warm_start.py`):
...
| kesit kalınlığının yuvarlanması | `wing_mesh.py:219` kesirli t/c geçiyor, basamak yok | **çürütüldü** |
...
birebir" maddesi üzerine bakıldı. `cfd/common/forces.py`'nin `hesapla()`
...
`forces.py`'ye `izdusum_alani(vaka, yama, eksen=1)` eklendi,
...
türetildi (`/tmp/setup_t3.py`): yalnızca U, k, ω ve Re_θt değişti,
...
C_f, `cfd/common/cf.py` ile elden hesaplandı (functionObject kırık).
...
**En savunulabilir C_D0 (wing/gövde, uçuş Re, tam türbülanslı):**
**0,0141 ± ~%5**, iki modelin duvar çözümlü/düşük y⁺ değerlerinin
ortası (SA 0,01475; k-ω y⁺≈20 0,01344).
...
kurulumda bozuk olduğu için `cfd/common/mesh_match.py` yazıldı: cKDTree ile
**en yakın komşu** ataması. Taşınanlar `nut`, `U`, `p`; k ve ω hedef
ağın **kendi duvar mesafesinden** yeniden kuruldu (`warm_start.py`).
...
yakınsamış alan duruyor. Yapıldı (`cfd/common/wall_pressure_compare.py`) ve sonuç
