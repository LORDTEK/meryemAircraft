# Round 60 — preparing to shorten: what each step must keep, where each claim lives, and which caveats cannot be cut; with last round's corrections and all fifteen steps

> **READ THIS FIRST.** You are starting a **new conversation**. Before this file, read
> **`reader-onboarding`** — it says what the paper is, what it claims and does not claim, which
> decisions are settled, which errors recur, and how to answer. **This file carries the round:**
> what was done with last round's answers (Section 1), the corrected passages (Section 2), this round's
> task and questions (Sections 3–4), and **all fifteen steps of the paper, in full** (Section 6).
> **Quote only from Section 6.** `paper-v6`, `paper-v7` and any earlier round text are not what this
> round is about.

`LORDTEK/meryemAircraft`, branch `claude/ecstatic-cori-6w30at`, commit **`8197f9f`** — metadata only, so that
a stale copy can be recognised. Section 6 is `paper/v8/ALL-STEPS.md` verbatim, with headings demoted one
level and a step label added; the Turkish audit tables under each step are the author's source ledger and
are left out.

```
cfd/reader-onboarding.md                      SHA-256 5f500e3c05a89babe2de62d41ca51fcb1de0bda3175b7c611cd6c07e2190f71e
...
paper/v8/03-the-escape-condition.md           SHA-256 250fcf61ac71a3a6a66857ce609b91901f5f43fd0ce42d804054fdc27d29fa7e
paper/v8/04-the-independent-check.md          SHA-256 553dacc28a42c1f936f60845e84d1e418f8220ec2b8f253f1385d8855fd56880
paper/v8/05-the-first-half.md                 SHA-256 9c657c197251fe1d0cd6bcd05e41897088328cf229ac87b528fa89cb86509160
paper/v8/06-the-second-half.md                SHA-256 bce67244399da2383e6f4d34b48237537d9f18b1354f91bf23e61a5cd83c5ddb
paper/v8/07-the-combination.md                SHA-256 80e4186a0203212cd8c0295977604188533c2a3a9007e69ab5870e30db4f6127
paper/v8/08-what-it-is-made-of.md             SHA-256 d246a1b7dbcaf58b79d0c84d2ac76c8c10040b3e0edd7c6c40045a064fbea4bc
paper/v8/09-what-is-not-claimed.md            SHA-256 c2247d941c3bbf673801ab825101cebb10c4882c42f0253eb34e003556d918bc
paper/v8/10-the-closure.md                    SHA-256 6f83c6a435370f7055a41741a7acf5dc12812cb4cf712ed2b34561e85f4ce4ed
paper/v8/11-the-ledger.md                     SHA-256 7e9c8969e61998a0e6cf5258818a4e835cec8b0663deb5ddb2db7bf468e74285
paper/v8/12-the-bills-separate.md             SHA-256 2c158533f8e45c95a9623285f11a1dcb3f696b05ec068911664783ec5e55515c
paper/v8/13-rankings-belong-to-contracts.md   SHA-256 8686d2e4710deb0f22e534f36796aaeb4ade8e09e397361a8f129cbaea5defd6
paper/v8/14-what-does-not-close.md            SHA-256 ddb2fd6964c747af6baf68361ea07dd24eb1e4032160182edfd3d92b67ef6190
paper/v8/15-four-axes.md                      SHA-256 7632e912912410bc6242c2a16f3a2be6c9a9692a94924b03ca840841878ff6be
paper/v8/ALL-STEPS.md                         SHA-256 aebd37973c164aab66f508f814811cb8f0a114f4139a5768f620a3922147edf3
```
...
**Grok, ChatGPT and DeepSeek answered** (Qwen did not this round). **All three:** last round's corrections
match their sources — the closures' geometry, the separation of the reference design's Bill 3 ratio (4.19)
from the closures' (2.4 to 3.2), and *"arranged to change regime"*. **No new mathematical contradiction was
found.** Three findings were real and larger than their authors framed them.

### 1.1 Step 3 contradicted Steps 7 and 8 — DeepSeek

Step 3's list of permitted costs said: *"the condition is about the propulsor that carries the aircraft, so
attitude hardware **does not violate it**."* Step 8, after last round's correction, says the tip pairs fall into
the **first** of Step 3's failure modes (carried through cruise producing no cruise thrust), and Steps 7 and 8
both treat the tip pairs as the part that fails. DeepSeek proposed the fix in Step 3, not Step 8. **Checked
and taken:** Step 3 was the outlier. It now says attitude hardware does not stop the propulsor that carries the
aircraft from meeting the condition, but it is carried through cruise without cruise thrust — the first failure
mode — and the architecture is a partial instantiation, the fourth.

### 1.2 The tip hardware does not scale with the closures — Grok, and one layer deeper

**Grok:** Step 10 scales wing area, span and nose disc with the closed mass, but leaves the tip-frame length,
the tip-disc diameter and the strip implicit — *"the same class as 1.1: a geometry quoted as if it belonged to
every closure."* Step 10 named the frames and the strip; **the tip-disc diameter was missing. Taken.**

**Checking it found more.** The frame and rotor drag terms (0.0043 and 0.0154 at the favourable end) are
**coefficients on the reference wing area of 1.979 m²**, and the closures carry them unchanged. That is the same
as letting the tip hardware grow with the wing. **Held at its reference size, that hardware would give terms 4 to
13 percent smaller across the four closures — 0.0009 to 0.0028 of zero-lift drag.** The closures do not take that
reduction; it has not been run through the loop. Step 10 now says so; the closure script prints it, and the
numerical check recomputes the text's figures.

### 1.3 "The light design" named different aircraft in different steps — ChatGPT and DeepSeek

In Steps 8 and 10 it meant the 50 kg reference design; in one place in Step 10 it meant the closures; Step 12
redefined it locally as the reference pair. **Taken:** Step 8 now defines *"the 50 kg reference design"* — the
design the inventory describes, re-closed at four masses in Section 10 and set beside a 1 000 kg reference design
in Section 12 — and Steps 5, 10, 11, 13 and 14 use that name. The bare *light design / heavy design* survives only
in Step 12, under its local definition, where it now means the same thing.

### 1.4 The rest

| Point | Who | Checked | Done |
|---|---|---|---|
| Step 13's tilt paragraph reads as a range ranking (*"leads under every contract at every closure — by 93 to 141 percent"*) beside *"no range claim"* | ChatGPT | Defensible as logic; the clearest residue on the fourth axis | Reworded: *"What the bound gives is a size, not an order"* — the margin is the room a real tilting aircraft's cruise penalties would have to fill; how much they fill is not computed |
| Step 15: *"producing moments rather than thrust"* — the tip pairs do produce thrust in hover | Grok | Right, and **Step 9 had the same words** | Both: *"rather than cruise thrust"* |
| Step 8's parenthesis still says what the tip pairs are **not**, and describes Step 3's second row too narrowly | Grok, DeepSeek | Right | The parenthesis now says only what they are |
| Step 14's table stub: 5.5 to 6.1 kW kg⁻¹ without its denominator | Grok | Right | Column heading: *"per kilogram of buffer"* |
| Step 6: *"between 52 and 58 kg"* | DeepSeek | Right | *"52.3 and 57.5"* |

**Declined, with reasons:**

- **DeepSeek: say that Step 12's 1 percent figure is "a pure scale result" and rest the separability argument
  on it.** Step 12 itself says that near-constancy is *"a property of the constant-disc-loading rule, not a
  finding about Bill 3."* The proposed sentence would contradict the paragraph it sits in.
- **DeepSeek: a forward reference in Step 7 to the failure mode named in Step 8.** Step 7's reason (the
  tip pairs are exposed in cruise and cannot be feathered) is consistent with Step 8; and nothing is being
  added to the text before it is shortened.
- **Grok: Step 12's Bill 3 paragraph cannot be quoted as a closure result.** Agreed; the paragraph already
  ends *"This paragraph compares the reference pair only."*

---

## 2. The corrected passages, verbatim

**Step 3, the clause that contradicted Steps 7 and 8:**

> Those are two different statements and the distinction matters: **attitude hardware does not stop the propulsor that carries the aircraft from meeting the condition, but it is carried through cruise without producing cruise thrust, which is the first failure mode below — and the charges are about everything the aircraft carries, so Bill 2 reaches it.** An architecture in that position is a partial instantiation, the fourth failure mode: it meets the condition where it carries the aircraft and still pays one of the three elsewhere. The condition permits such hardware outside the first charge and does not make it free.

**Step 8, the parenthesis:**

> *(They are sized for moments and used for them in both regimes; they add the take-off margin (Section 5) but were not sized for weight support. Section 3's permitted-cost clause therefore places them outside the first charge while leaving them in the airstream.)*

**Step 10, what the loop does not scale, and the drag terms:**

> **Three things the loop does not scale, and a reader comparing this section with Section 8 should know which is which.** The tip-frame length, the tip-disc diameter and the strip are not sizing variables here. They were set on the reference geometry — the 50 kg reference design of Section 8 — and **the control moment arms of Section 8 are therefore reference values that this closure does not re-derive.** Section 8 describes one aeroplane; this section describes what its sizing rules give at four sets of inputs. **These are the same configuration at four closed masses rather than four configurations** — but anything that depends on the arms is carried at the reference geometry and is not an output of the loop.
>
> **The frame and rotor drag terms are carried the same way.** They are coefficients on the reference wing area of 1.979 m², and holding them unchanged across the closures is the same as letting that hardware grow with the wing. Held at its reference size instead, it would give terms 4 to 13 percent smaller across the four closures — 0.0009 to 0.0028 of zero-lift drag. **The closures do not take that reduction, and it has not been run through the loop.**

**Step 13, the tilt bound:**

> **What the bound gives is a size, not an order.** A tilting layout credited with no cruise penalty at all is 93 to 141 percent ahead of this configuration under every contract at every closure, and moving from the first contract to the third shifts that by 1 to 18 points toward this configuration. **That margin is the room a real tilting aircraft's cruise penalties would have to fill** — nacelle drag, pivot fairing, hover-sized rotors flown as cruise propellers — none of which is modelled here, and how much of it they fill is not computed.
>
> **There is a trade, but it is lopsided.** The tilting layout closes 0.5 to 5.4 percent heavier than this configuration, and it cruises at the clean airframe's lift-to-drag ratio with a propeller at 0.80: it is credited with no nacelle drag, no pivot fairing, and no penalty for flying hover-sized rotors as cruise propellers. **Even the contract that weights mass most** — a fixed take-off mass, in which every kilogram of tilt mechanism is a kilogram of fuel not carried — **leaves the bound's margin at 93 to 130 percent.** The contract moves the comparison, as Section 12 says it must where there is a trade; none of the three moves it far enough to matter. A ranking against a competitor modelled as a bound is not a ranking, and **no range claim is made against the tilting family in either direction.** The claim this paper makes against that family is about mechanism (Sections 7 and 8), and nothing in this section bears on it.

---

## 3. This round: preparing to shorten

**The author has decided to begin shortening, gradually, and wants one round of preparation first. Nothing is
cut in this round.**
...
| | Now | Working target |
|---|---:|---:|
| Text | about 30 100 words in fifteen steps | about 7 500 words |
| Tables | 16 | 8 |
| Figures | none in the steps | 6 |

The journal counts each figure or table as 200 to 700 words inside a 10 000 to 12 000 word limit. Supplemental
files are allowed, but the journal says the article *"must be self-contained and stand on its own. Acceptance for
publication will be based solely on the content of the article."* **Whatever moves to a supplement cannot be
something the body's claims need.**

**This is not trimming.** A quarter of the length means that most steps cannot survive as sections, and that
several claims now stated in six or seven places will be stated once. **The risk this project knows best is that a
shorter sentence is a stronger one**: the qualifications that keep each claim at its strength are the words most
easily cut. The preparation is meant to find those before anyone cuts.

**What constrains the answer — the author's standing decisions:**

- **The innovation is the main line and must not be overshadowed; the calculations stay, in their right place.**
  Neither is bought at the other's expense.
- **One contribution: the architecture.** The framework is the instrument that makes it checkable.
- **The author's outline of the argument:** *introduction · the current state · the solution to one problem · the
  solution to the other · combining the solutions (a move of its own) · why the result holds · the calculations ·
  conclusion.*
- **Coarsest first.** The author has said before that a flow must be agreed before word budgets, table counts or
  figure assignments are built on it. **This round asks for the coarse map, not for word counts.**
- **One decision needs the author, and is being asked.** An earlier decision says the framework steps (2, 3, 4,
  12, 13) *"keep their own steps at full size."* It was made when shortening was off the table, and a body a
  quarter of the present length cannot keep any step at its present size. **Until the author says how it applies
  now, please state which reading your proposal assumes:** the framework shortened in proportion with the rest, or
  protected relative to the rest.

**What readers have already recorded as repeated** (last two rounds, from Grok, DeepSeek and Qwen) — offered as
material, not as a verdict:

- the declined reaction-torque channel — Steps 1, 5, 7, 8, 9, 14, 15, *"at the same strength"* (Grok);
- partial instantiation — 3, 7, 8, 9, 11, 15;
- the transition not shown — 1, 5, 7, 8, 9, 10, 14, 15;
- the duty-cycle mismatch (*"about two percent of the flight"*) — 2, 3, 11, 15;
- the mechanism-class list — 7 and 15; no range claim against the other hybrids — 9, 13, 15; *"by construction
  means by the sizing"* — 9, 15;
- the fixed-pitch efficiency gap — 6, 11, 12;
- the parallel structure of Steps 5 and 6; Step 11's opening and closing; the end of Step 14 and Step 15's
  *"Where the paper stops"*; Step 1's 1954 history beside its *"what is already occupied"*.

---

## 4. What I am asking

1. **The core of each step.** For each of the fifteen steps, in one or two sentences: **what the argument loses
   if this step disappears** — the claim, the number or the qualification that nothing else in the paper
   carries. If the answer for a step is "nothing that another step does not also carry", say so and name the
   other step.
2. **One home for each repeated claim.** For each item in the list above, and any you add: **which step should
   carry its full statement**, and whether the other places need a pointer, a clause, or nothing.
3. **The caveats that cannot be cut.** Which qualifications, if removed, would turn a claim into an over-claim?
   **Quote each one and name its step.** This is the most important question of the round: it is the list that
   will be checked against every shortened draft.
4. **Body or supplement.** Which material can move to a supplemental file without the article ceasing to stand
   on its own — and **which must stay in the body for each of the four axes to be checkable**? Name the steps or
   the paragraphs.
5. **The coarse map.** Map the fifteen steps onto the sections of a journal paper that follows the author's
   outline: which steps merge, which become a paragraph of another section, which move to the supplement.
   **Sections and their order only; no word counts.** Please make *combining the solutions* visible as its own
   move in your map.
6. **Tables.** Of the sixteen tables in the steps, which ones change a reader's belief about one of the four
   axes? (The working criterion: an object that does not do that does not stay in the body.)
7. **This round's corrections** (Section 2): stronger or weaker than their sources, or in contradiction with
   another step?

**Sources.** None of this needs a source. PDFs only for priority claims, numbers taken from tables, and verbatim
quotations — and please say which document you opened in this conversation for any number you give.

---

## 5. How to read Section 6

**Step N is Section N of the paper**; cross-references in the text (*"Section 10"*) point to the step of that
number. Each step's label is added in brackets above its heading. Nothing else in Section 6 has been edited for
this text.

---

## 6. v8, all fifteen steps, in full
...
## 7. The questions again, in one place

1. **The core of each step** — what the argument loses if it disappears.
2. **One home for each repeated claim**, and what the other places keep.
3. **The caveats that cannot be cut** — quoted, with their steps. *The most important question of the round.*
4. **Body or supplement** — and what must stay in the body for each axis to be checkable.
5. **The coarse map** — sections and their order, with *combining the solutions* as its own move; no word counts;
   say which reading of the framework decision you assume.
6. **Tables** that change a reader's belief about one of the four axes.
7. **This round's corrections** in Section 2.
