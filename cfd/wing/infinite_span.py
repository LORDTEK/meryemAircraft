...
ve turbulans kurulumu naca/setup.py'den, ikisine de ayni sekilde geliyor
(bkz. setup3d.py).
...
from wing_mesh import duz_kanat                        # noqa: E402
from setup3d import kur3b                               # noqa: E402
...
def kos(vaka, cek=4):
    subprocess.run([os.path.join(BURA, "run3d.sh"), vaka, str(cek)],
                   check=True)
    ...
    with Kilit(KOK):
        ...
        if "2b" not in yapilan:
            ...
            if son_adim(v) is None:
                kur2b(v, **ORTAK)
                subprocess.run([os.path.join(BURA, "..", "naca", "run.sh"),
                                v, "4"], check=True)
            else:
                subprocess.run([os.path.join(BURA, "..", "naca", "continue.sh"),
                                v, "3000", "4"], check=True)
