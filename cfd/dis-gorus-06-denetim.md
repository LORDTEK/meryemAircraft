...
| y⁺≈1 | 0,014750 | **0,012532** |
| duvar etkisi | +1,58% | −6,74% |

Model açılımı y⁺≈1'de **%18**. Ortalama alınmıyor; iki değer ayrı
veriliyor, aralık "spread" diye etiketleniyor.

Kontrol koşuları: `blended false` → 0,012530 (%0,016 fark, önemsiz).
Devam eden: y⁺≈20 çözümünden taşınmış başlangıç (bağımsızlık testi).
...
- k-ω y⁺=1 çözümü **ancak ısınmış başlangıçla** elde edilebildi;
  başlangıç bağımsızlığı gösterilemedi (devam eden koşu bunu sınıyor).
...
2. **Gördüğünüz ama benim listemde olmayan bir hata var mı?** Asıl
   aradığım bu.
3. **Sonraki aşama ne olmalı?** Elimde: temel modelinin ağır hatta
   (1000 kg) koşulması, makaleye bir bölüm olarak yazılması, başlığın
   "Eliminating" → "Reducing" değişimi, 6-DoF geçiş benzetimi, kütle
   bütçesinin bileşen düzeyinde kapatılması. Sıra ne olmalı — ve
   bunlardan hangisi Q1 için **gerçekten** gerekli?
