# Round 49 — the whole of steps 1 to 9, at last; and the author was right that a finding was waiting

> **READ THIS FIRST.** Everything this round asks about is reproduced in this message.
> **You are not being asked to read a manuscript.** This project does not edit manuscripts. v8 is
> written as separate step files in `paper/v8/`; `paper-v6.md` and `paper-v7.md` are frozen
> historical records. **If your knowledge base holds any file whose name contains `makale-v` or
> `paper-v`, it is not what this round is about, and consulting it will mislead you.**

`LORDTEK/meryemAircraft`, branch `claude/ecstatic-cori-6w30at`, commit **`e554165`**.
`paper/v8/ALL-STEPS-1-9.md` SHA-256 `1f3aab132e215031873a3009979981bac4df96143d59975ebd3607a2727d0b39`.
Hashes are for checking a file you happen to have, not for finding one.

---

## 0. An admission that is overdue

**Step 9 has never been sent to you.** It was written, it has been quoted at you, its conclusions
have been relayed — and the text itself was never put in front of any of you. The same is true of
steps 2, 3, 7 and 8 since they were first written. You have been asked for opinions on a paper
while being shown one section at a time.

**The author put it plainly, and he is right:** *"Asking for an opinion without giving the
section you prepared is its own kind of stupidity."*

The cost is measurable. Last round I retired the phrase *"different efficiency class"* from step 6
and wrote out the reasoning. **It was still alive in step 9**, in the claim table, in the row
marked **Claimed**. None of you could have caught it. It was found only because I happened to open
step 9 for another reason.

**CLAUDE.md §2 has said "ask for a part–whole–part reading before a major stage" since round 20 or
so, and I have never once done the "whole".** This round does it. **Steps 1 through 9 are
reproduced below in full**, Turkish audit apparatus stripped, in the order the argument runs.

---

## 1. The author's two decisions, and the audit that changed one of them

### 1.1 Span efficiency — author's preference: **0.817**

His reason: *"use 0.817, because I remember you computed that number several times."*

**I checked the memory before acting on it, and it is correct — but the conclusion is not the one
anyone expected.**

`aero/drag_sweep.py`, line 40:

```python
CL, E_SPAN = 0.45, 0.817
```

**All three lift-to-drag values in the bracket — 11.88, 10.82 and 8.80 — were produced with the
computed span efficiency, not the assumed 0.85.** The script's own construction test passes: at
the published C_D0 it reproduces the published clean-body 13.40 against 13.44, a 0.33 percent
difference. And `baseline.py`, which closes the mass loop, has no span-efficiency parameter at
all — it takes lift-to-drag as an input, so `e` enters the chain only through this one place.

**So Grok's concern is inverted.** He wrote: *"If step 10 uses 0.85, the polar in step 6 is
already the optimistic one."* The polar is not the optimistic one. It is the computed one, and it
has been all along.

**And that exposed a false caveat in step 6, which is now removed.** The section's
"not demonstrated" list said:

> ~~*"The span efficiency of 0.85 is an assumption which the paper's own calculation puts at 0.817
> — optimistic by 3.9 percent."*~~

That sentence is true of the v7 baseline chain. **It is not true of the numbers step 6 quotes**,
which already carry the penalty — so, sitting beside them, it told the reader our figures were
3.9 percent optimistic when they were not. It now reads:

> *"The span efficiency used throughout this section is the computed value, 0.817, not the assumed
> 0.85 — a vortex-lattice solution of the trimmed planform, and 3.9 percent below the assumption,
> so the lift-to-drag figures above carry the calculated penalty rather than the optimistic
> estimate."*

**The author predicted this before the round was written**, in these words: *"I'm writing this
here before it happens — a finding will definitely be found."* It was, and it was found by
checking his own instruction rather than by accepting it.

**What is left to ask you is therefore a different question, and a sharper one.** The drag sweep
exists because three different zero-lift drag values were circulating: 0.0248 in Sections 3.7 and
3.8, 0.0401 in Section 3.6, and a bracket in the supplement. The sweep reduced them to a
consistent bracket of **0.0285 to 0.0381**, and noted that **the published 0.0248 lies below both
ends** — it is not inside the bracket at all. **Step 10 closes the sizing loop. Which C_D0 should
it close on?** The consistent bracket, reporting a range; or the published single value, which the
sweep says is unsupportable. This is the real version of Grok's question and it needs answering
before step 10, not after.

### 1.2 The blade family — author's preference: **keep it as an envelope**

ChatGPT argued that step 10 should eventually select one blade family and demote the other three
to sensitivity, while agreeing that the envelope is correct until step 10 exists. **The author
prefers the envelope**, and step 6 currently holds it that way: two readings are reported, the
examined envelope 5.56–7.39 and the best examined family 6.01–7.39, and **neither is presented as
the aircraft's performance.**

The question to you: **does the envelope survive step 10?** A closed sizing loop needs one
propeller, because mass feeds back through the loop and four blade families give four closures.
Either step 10 carries all four through and reports four closures, or it selects one and the
envelope collapses. **If there is a way to keep the envelope honest through a closed loop, say
how** — that is the author's preference and I do not yet see the mechanism.

---

## 2. What else changed since you last saw step 6

Briefly, because the full text is below.

- **"A designer would take the best of them" is gone** (ChatGPT). Blade count and section loading
  also govern structural loads, acoustics, the motor operating point, rotor inertia and
  manufacture, **none of which is modelled**, so the highest-efficiency family is the best only on
  the one criterion examined.
- **Both readings of the margin are now printed** (Grok): against the examined envelope,
  +14 % … +51 % and −4 % … +27 %; against the best examined family, **+23 % … +51 % and
  +4 % … +27 %**. In the best family the sign holds against both references.
- **Three overbroad sentences narrowed** (ChatGPT): *"a rotorcraft's discs"* → *"a multirotor's
  discs"*; *"the surface costs no power"* → *"the wing produces its lift without a separate
  continuous power supply of its own"*; *"not reliable for anyone on this class"* → *"for the
  methods used here"*.
- **A fifth qualification added** (Grok): the atmospheres are not matched — the published sizing
  mission is flown at *"5,000-ft altitude and ISA + 20°C"* and every number here is sea level.
  **Direction not claimed**, because it has not been computed.
- **The direction of the speed mismatch was computed** (DeepSeek), not asserted: `L/D_max` exceeds
  the cruise ratio at both ends of the bracket — 11.65 against 10.82, and 10.08 against 8.80, both
  at e = 0.817. **The reference is given its best speed, this configuration is not, and the margin
  is positive anyway.**
- **Step 5:** *"the landing gear's reaction surface"* replaced — the ground is inevitably the
  reaction surface; the requirement is that the site supply **no prepared infrastructure**.
  *"They close"* became *"Section 10 reports **whether** they close"* (Grok: step 5 cannot
  pre-announce step 10's result). **Closed-loop hover control added to the not-demonstrated list.**
- **Step 9:** two new sections — what each claim does **not** depend on, and the unpriced cost of
  declining the reaction-torque channel.
- **DeepSeek's proposed fifth qualification was rejected**, with the source: §6.3 says *"For the
  quadrotor, both collective and rotor speed control were considered … The increase in C_T/σ might
  be limited by maximum blade loading, perhaps requiring a smaller design C_T/σ at hover."* That is
  a two-regime compromise. The quadrotor's rotors hover **and** cruise.

---

## 3. What I am asking of you this round

**This is the "whole" reading, and it is the one thing I want from it:** you have judged these
sections one at a time, each in isolation, over many rounds. **Read them as one document and tell
me what breaks across the seams.** Specifically:

1. **Contradictions between steps.** The "different efficiency class" survival in step 9 is one
   instance of a class of error, and it was invisible while you saw one step at a time. **Are
   there others?** A claim narrowed in one step and still broad in another; a number given two
   ways; a qualification present in one place and absent where it matters more.
2. **Does the argument actually run?** Gap → tax → escape condition → independent check → first
   half → second half → combination → inventory → boundary. Read end to end, does each step earn
   the next, or does something arrive before it has been paid for?
3. **Is anything claimed in an early step that a later step quietly withdraws** — or the reverse,
   anything withdrawn early and then relied on later?
4. **Proportion.** Nine sections, roughly 15 000 words. Is the weight in the right places for a
   paper whose single contribution is the architecture?
5. **The two decisions in section 1** — the C_D0 question for step 10, and whether the blade
   envelope can survive a closed loop.

**No source is needed for any of this.** §2.1's boundary stands: PDFs are requested only for
priority claims, numbers taken from tables, and verbatim quotations.

**The author's standing instruction for this round:** *"I don't want to move to the next stage
while there is an error or a problem. One more round costs nobody anything."* He also said, before
this text was written, that a finding would certainly be waiting. One was. **If you find the
second one, that is the round doing its job.**

---

# STEPS 1 TO 9, IN FULL

*Nine sections, in argument order, Turkish audit tables removed. This is the current text at
commit `e554165` and it supersedes every earlier version of any of these sections.*
