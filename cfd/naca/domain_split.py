...
BULGU. Yerlesik alan taramasi (domain.py) n_normal'i her R icin YENIDEN
...
"saf alan taramasi" diye bir sey yoktur ve domain.py'nin buyume oranini
    ...
    with Kilit(KOK):
        ...
        for ad, R, nn in VAKA:
            ...
            if not os.path.isdir(os.path.join(v, "processor0")):
                ...
                subprocess.run([os.path.join(BURA, "run.sh"), v, "4"], check=True)
            else:
                subprocess.run([os.path.join(BURA, "continue.sh"), v, "3000", "4"],
                               check=True)
