...
# kaldi. run.sh vakayi bastan kurar -- processor* dizinlerini ve agi
...
#
#   kullanim:  continue.sh <vaka-dizini> <yeni-endTime> [cekirdek]
set -e
...
if [ ! -d processor0 ]; then
  echo "continue.sh: processor0 yok -- bu vaka ayristirilmamis, run.sh kullanin" >&2
  exit 1
fi
