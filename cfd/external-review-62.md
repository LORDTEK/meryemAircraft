# Round 58 — all fifteen steps, in full, for the first time; the contribution is now named on the first pages

> **READ THIS FIRST.** This text contains **the whole of v8 as it now stands — all fifteen steps, in
> full** — together with an account of what was done with last round's answers. **You are not being
> asked to read anything else, and you do not need the repository.** `paper-v6.md` and `paper-v7.md` are
> frozen historical records. **If your knowledge base holds a file whose name contains `makale-v` or
> `paper-v`, it is not what this round is about — please quote only from Section 5 below.**

`LORDTEK/meryemAircraft`, branch `claude/ecstatic-cori-6w30at`, commit **`94e4299`** — metadata only, for
anyone who wants to check that the text below is the text in the repository. Each step file carries a
Turkish audit table under its English body; those tables are the author's source ledger and are left out
here. Section 5 is `paper/v8/ALL-STEPS.md` verbatim, with headings demoted one level and a step label added.

```
paper/v8/ALL-STEPS.md                         SHA-256 a52e1e808a5b4793358b2dcf469b3713ee2ffea7f88078709d14a7aed05e8385
...
paper/v8/08-what-it-is-made-of.md             SHA-256 bed6c0c449a5f1c04d47905b66755d9db2c5b50dc616dfb817bc01aa7e0bd82b
paper/v8/09-what-is-not-claimed.md            SHA-256 f6519fe17915546a2ec5351184ce4ef78395930946afe2cc03227cc4c43b5fc8
paper/v8/10-the-closure.md                    SHA-256 48aed0616ba2b119eeccdabc3f5562e64e17dc0cae3b58eac07bf33bbb4b440c
paper/v8/11-the-ledger.md                     SHA-256 fc928952d8549ba79854dabf14a72009b52f3f06f08fb5998bc6168d59a45f60
paper/v8/12-the-bills-separate.md             SHA-256 99ecbe44f52171b08bebc04c70ff7bdeb16d4115ce0df62c26c4bd3809c08adf
paper/v8/13-rankings-belong-to-contracts.md   SHA-256 bf34c8dae4342541fb97928d2439750d1ee7cc6616f800fee695443eb449be2a
paper/v8/14-what-does-not-close.md            SHA-256 2c863c86e83fd70fbdf72c74f54fd3ac391747a86d6e1a4e28abba2fcaf3c643
paper/v8/15-four-axes.md                      SHA-256 55f00358d3a9de29f73f727e4fc7f0b18edb434b1eabf67575c4d560f533976e
```

**Where the work stands.** v8 is being rebuilt, not shortened, for *Journal of Aircraft*. All fifteen
steps are now written. **This is the first round in which you see them together**, and so it is the
round for a **part–whole–part reading**: read the whole once for the argument, then go back to the
parts and check each against the whole. **Shortening is not on the table** — the author's instruction
stands: *"There will be no shortening until I am confident."* The length problem is known (the body is
about three times the journal's limit) and is recorded, not being worked.

**Qwen**, last round you were promised all fifteen steps. They are in Section 5. Last round you worked
only from the v8 text and brought a correct item from Step 8 (the hover torque residual) — thank you.

**What the paper claims, on one line, so that you can check every step against it:** a configuration
**sized** to combine runway-independent vertical operation with wing-borne cruise efficiency,
**arranged** to do so with no mechanism that reorients a propulsor, and an account of what the
combination costs. **The single contribution is the architecture** — the mechanism axis against tilting
architectures. Cruise efficiency is claimed against multirotors only; runway independence against
fixed-wing aircraft only, as sized and not demonstrated; **range against the other hybrids is not
...
**All four of you agreed:** Step 15 is faithful to its sources; the mechanism axis is at its narrow
strength; P1 and P3 are right; Step 7 should not be grown; Step 14's *"the loop closes"* holds.

| Point | Who | Done |
|---|---|---|
| The closing sentence *"a configuration that combines runway-independent vertical operation …"* asserts what is only sized | ChatGPT (*"mandatory"*) | **Taken, in Step 15 and in Step 9**, which carried the same sentence: *"a configuration **sized to combine** runway-independent vertical operation with wing-borne cruise efficiency, **arranged to do so** with no mechanism that reorients a propulsor, and an account of what the combination costs"* |
| Steps 9 and 15 list different things the mechanism claim does not depend on | Grok, DeepSeek | **Equalized.** Both now: the drag bracket, the propeller efficiency, the sizing contract, the range result, the energy store, the transition aerodynamics |
| Step 7: *"not currently reliable for anyone"* — a retired universal had come back *(my record of this round said Step 8; it is Step 7)* | Grok | **Taken**: *"for the methods used here and the published comparisons against which they were checked"* |
| Step 6: *"at e = 0.85 … 7.48"* — a symbol collision (e is span efficiency elsewhere) and an unswept 7.48 | Grok, DeepSeek | **Taken**: *"η_p = 0.85 … 7.47"* |
| Step 15's mechanism list: a variable-pitch hub and dedicated lift rotors do not *reorient* anything | Grok | **Taken**: *"— nor any of the other mechanism classes that architectures use to change regime or to take a rotor out of one regime's flow"* |
| *"One of the measured continuous ratings"* is ambiguous | DeepSeek | **Taken**: *"the unit pack's continuous rating"* |
| Step 5: *"the difference between that peak and the cruise demand"*, with no word about the store | DeepSeek | **Taken**: *"what the engine cannot deliver of that peak — at a specific power Section 14 examines"* |
| Step 15 compresses the unknowns | DeepSeek | **Taken**: *"Section 14 gives the full list"*; one sentence on certification |
| Step 14's table: the blade-family selection criteria | ChatGPT | **Added** — Step 10's own list |
| Step 14's table: the hover torque residual | Qwen | **Added**, to the hover-control row |
| Step 14's atmosphere row: its own sensitivity was computed; its effect on the Step 6 comparison was not | DeepSeek | **Added** |

**Declined, with reasons:**

- **DeepSeek: a marginality note on the 335 kg row.** The table and the text already say *"set by
  nearness to non-closure"* and *"only just closes"*.
- **DeepSeek (optional): name the four axes in Steps 1 and 2.** The skeleton is the author's; the question went to the author together with P1,
  and the author's instruction was to apply P1 and P3.

All of these are in the text in Section 5. Every retired phrase is in an automated list that scans all
fifteen English bodies; the list rejects the old text when tested against it.

---

## 2. P1 and P3 are applied — and what I changed in them before writing

**The author's decision this round:** *"Apply P1 and P3."* Last round's proportion pass found that the
contribution's name — *"no mechanism that reorients a propulsor"* — first appeared **57 % of the way
through**, in Step 7, and that the calculations (Steps 10–14, over 40 % of the text) barely returned to it.
**It now first appears at 6.5 %**, at the end of Step 1.

**I drafted the sentences for the author, and the author approved them. Writing them into the text, I
changed four of the five. Here is each draft, what was written, and why — so that you can judge whether
the change was right.**

| Where | Draft the author approved | Written | Why it differs |
|---|---|---|---|
| **Step 1, end** | *"The contribution is the architecture: a configuration **that changes regime** by rotating the airframe rather than its propulsors, and so carries no mechanism that reorients a propulsor."* | *"**The contribution is the architecture: a configuration arranged to change regime by rotating the airframe rather than its propulsors, and so carrying no mechanism that reorients a propulsor.** The combination, the consequences of the choices inside it, and an accounting of what they cost are how that contribution is presented and priced — not a claim that the route was waiting to be found."* | **My draft over-claimed.** *"That changes regime"* says the aircraft does it; Steps 7, 9 and 15 say the transition is **not** shown. *"Arranged to"* is the strength the closing sentence already uses. |
| **Step 9** | — (not in the plan) | *"**The contribution is the architecture, and the paper presents it as** the combination, the consequences of the choices inside it, and the accounting"* | **Propagation.** Step 9 stated the three items as *"what this paper offers"* — exactly what P1 reframes. Left alone, Step 1 and Step 9 would have said different things. **This is the error class that has recurred in this project: a correction that does not reach every place carrying the old claim.** It was caught before sending, by searching the fifteen bodies for the phrase. |
| **Step 10, opening** | *"This section prices the arrangement of Sections 7 and 8 on a declared package; it does not prove the count, and it does not show the aircraft can be built."* | *"This section prices the arrangement of Sections 7 and 8 on a declared package; it does not bear on the count of mechanism classes, which rests on the inventory of those sections alone."* | *"Can be built"* is said two sentences later in the section's own words. *"The count"* read without a referent. |
| **Step 11, opening** | *"The ledger attributes that price to the three charges; it adds none."* | *"Like the closure, the ledger prices the arrangement; the count of mechanism classes is not an entry in it."* | *"It attributes. It does not add."* opens the section's next paragraph. The draft repeated it and did not say where the ledger stands relative to the contribution — which was P3's purpose. |
| **Step 12, opening** | *"This section tests whether the charges the ledger uses are three quantities or one; it does not bear on the mechanism claim."* | *"Either answer leaves the mechanism claim where it was; that claim rests on the inventory of Sections 7 and 8."* | The first half repeated the section's opening question. |
| **Step 13, opening** | *"… the mechanism claim is not a ranking and is not at stake here."* | *"**The mechanism claim is not a ranking and is not at stake here**; what is at stake is how the price computed in Sections 10 and 11 enters a comparison with other architectures."* | Substance unchanged. |

**What I checked each new sentence against.** Step 1's: Step 7 (*"The change of regime is then made by
rotating the airframe"*), Step 7's table of mechanism classes, Step 15. Steps 10–13's: Step 9's table of
what each claim does not depend on, and Step 15's *"It rests on the inventory of Sections 7 and 8."*
**One loose end I did not resolve and want you to see:** Step 9 says the mechanism claim is *"settled by
the inventory in Section 8"*; Steps 12 and 15 say *"Sections 7 and 8"*. Section 7 counts the classes and
Section 8 lists the hardware, so both are defensible, but they are not the same words.

**The proportion now** (English bodies only):

| Flow element (the author's outline) | Steps | Words | Share |
|---|---|---:|---:|
| Introduction | 1 | 1 989 | 6.7 % |
| Current state — the framework | 2–4 | 5 924 | 19.9 % |
| One problem solved — runway | 5 | 1 245 | 4.2 % |
| The other problem solved — cruise | 6 | 2 230 | 7.5 % |
| **Combining the solutions** | **7** | **1 355** | **4.6 %** |
| Why the result holds — inventory and boundary | 8–9 | 3 706 | 12.5 % |
| Existing calculations | 10–14 | 12 284 | 41.3 % |
| Conclusion | 15 | 1 020 | 3.4 % |
| **Total** | | **29 753** | |
...
**This is the first time you can read the whole.** The questions are ordered from the whole to the parts.

1. **The whole, once.** Read Section 5 through. **Does the argument run — introduction, the price any
   architecture in this corner pays, one problem solved, the other solved, the solutions combined, why
   the result holds, the calculations, the conclusion?** Where does it stall, loop back, or say
   something a second time that it has already said? **You are not being asked what to cut** — shortening
   is not being done — but where a step repeats another, say which and where; that is a record for later.
2. **Contradictions between steps.** The error that has recurred most in this project is a predicate in
   one step that another step contradicts — usually after one of them was corrected. **Now that all
   fifteen are in front of you, please look for exactly that**: the same quantity, claim or strength
   stated differently in two places. Name both places and quote both sentences.
3. **The contribution, now named in Step 1.** Is P1 at the right strength — *"arranged to change
   regime"*? Is the contribution **overshadowed** anywhere after Step 1 — a step that reads as if the
   paper were about range, or about the framework? And do the four P3 sentences in Steps 10–13 do their
   job without claiming anything?
4. **The combining step.** The author's outline marks *"combining the solutions"* as a move of its own,
   not a by-product. **Does Step 7 read as that move**, in the context of all fifteen? It is short, and it
   is not being grown to fix its share; the question is whether it carries its weight.
5. **The four axes against the whole text.** Is there any sentence, in any step, that competes with
   fixed-wing aircraft on range, with multirotors on vertical capability, or with the other hybrids on
   range — or that says *"simpler"*, *"no moving parts"*, or claims the transition is shown?
6. **Numbers across steps.** Any figure quoted in two steps with different values, different stations
   (rotor shaft, engine shaft, electrical bus) or a different contract?
...
The step files are numbered 1 to 15, and **Step N is Section N of the paper**; cross-references in the
text (*"Section 10"*) point to the step of that number. Each step's label is added in brackets above
its heading. Nothing else in Section 5 has been edited for this text.
...
1. **The whole, once:** does the argument run, and where does it stall, loop back or repeat? (A record
   for later, not a request for cuts.)
2. **Contradictions between steps:** the same quantity, claim or strength stated differently in two
   places — name both and quote both.
3. **P1 and P3:** is *"arranged to change regime"* the right strength; is the contribution overshadowed
   anywhere after Step 1; do the four P3 sentences claim nothing?
4. **Step 7** as the combining move, read inside the whole.
5. **The four axes** against every sentence in all fifteen steps.
6. **Numbers across steps:** same figure, different value, station or contract.
