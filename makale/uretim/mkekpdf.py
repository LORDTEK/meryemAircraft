# -*- coding: utf-8 -*-
"""EK MALZEMEYI PDF'E BASAR -- makale/pdf/meryemAircraft-ek.pdf
...
da oyle. Makalenin PDF'i mkmakale.py'den cikiyor, bu betik ayni CSS ile
...
def stil():
    """mkmakale.py'nin CSS'ini calistirmadan ceker."""
    s = open(os.path.join(BURA, "mkmakale.py"), encoding="utf-8").read()
    m = re.search(r'^CSS = """(.*?)"""', s, re.M | re.S)
    if not m:
        sys.exit("mkmakale.py icinde CSS bulunamadi")
    return m.group(1)
