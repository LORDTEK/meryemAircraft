...
**The fairing is not only a drag measure, and Section 4.4 says why.** The frames are the only surfaces on this aircraft standing perpendicular to the wing plane, and the planform supplies no directional stability at all. The fairing is therefore also the vertical surface that provides it. Sized against the criterion the tailless literature recommends — C_n_β greater than 0.001 per degree, or 0.0573 per radian [19] — the chord required over the combined frame length is 39 mm, against the 50 to 70 mm a 20 mm faired strut carries in any case. The two requirements therefore do not conflict, and the directional one is the looser of the two; but the frame cross-section is now constrained from two directions, and a selection made on drag alone would be made on half the evidence.
