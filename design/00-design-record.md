...
| N103 | ✅ **Doğrulama betiği yazıldı** — `paper/build/verify.py`. Makalenin her başlık sayısını, makalenin kendi denklemleriyle bağımsız hesaplayıp metinle karşılaştırıyor: 33 kontrol + geçiş tablolarının 52 hücresi. Bayat sayı bir daha sessizce kalamaz |
...
| N108 | 🔴 **`transition2.py` kaldırma eğrisi eğimini %18 fazla varsayıyor:** 2π/(1+2/AR) = 4,72 /rad, VLM 3,87 /rad. Yön kötü tarafa — benzetim kaldırmanın gerçekte olduğundan hızlı toparlandığını sanıyor |
| N109 | ✅ **Ama Bölüm 7.4 buna dayanıyor.** İki eğimle de tablolar üretildi (`aero/sensitivity.py`): yayımlanmış değerlerde en çok **1,2 m** değişiyor, ve **referans profillerin ikisi de (hafif 2 s, ağır 4 s, w₀=5) her iki eğimde de sıfır irtifa kaybı** veriyor. Sav sağlam |
...
| N117 | ✅ **Menzil yöntemi düzeltildi (N114'ün karşılığı).** Seçenek tartışıldı: (a) seyir hızını azami L/D noktasına çekmek — hafif hatta 25,3 m/s, stall'ın yalnız **1,26 katı**, tasarım payı için fazla yakın; (b) seyir hızını korumak ve L/D'yi seyir noktasındaki polardan hesaplamak. **(b) seçildi.** Yöntem artık C_L = W/(qS), C_D = C_D0 + C_L²/(πARe), L/D = C_L/C_D |
| N118 | **Düzeltmenin yayıldığı yerler:** hafif hat L/D 12,7 → **12,0**, menzil 1 695 → **1 598 km**, dayanım 15,7 → **14,8 h**; ağır hat 14,0 → **13,6**, 1 868 → **1 814 km**, 13,0 → **12,6 h**. L/D düşünce seyir gücü yükseliyor: hafif 1,6 → **1,7 kW** elektrik ve 1,8 → **1,9 kW** motor mili; ağır 38,1 → **39,2 kW** ve 44,5 → **45,8 kW**. Motor dereceleri (2,6 / 54,3 kW) hâlâ yeterli. Şekil 12'nin işaretçileri ve `verify.py` de güncellendi |
| N119 | ⚠️ **Depo kaynağı artık yayımlanmış v1'den ileride.** Zenodo'daki v1 değişmez; düzeltme v2 ile gidecek. README'ye uyarı kondu ki depoya giren biri PDF ile kaynak arasındaki farkı hata sanmasın. `verify.py`'ye yeni bir kontrol eklendi: L/D'nin gerçekten seyir noktası polarından geldiğini sınıyor — 40 kontrol, sıfır sapma |
| N120 | ✅ **`C_D0` politikası (tasarımcı onayı):** 0,0248 **varsayım olarak kalır**; hesap onu değiştirmez, **sınırlar**. Gerekçe: beyan edilmiş kötümser bir varsayıma saldırılamaz, ama hesap sonucu yazılırsa hakem hesabı denetler ve en zayıf halkamız (2B şerit yöntemiyle %25 kalınlıkta merkez gövde) açığa çıkar. Menzilin %2'si için bu takas yapılmaz |
| N121 | ✅ **Uç göbek çapı piyasadan bağlandı.** Her rotor 8 inç pervanede 0,83 kgf / 168 W vermeli → 22 mm statorlu standart sınıf. **T-Motor MT2216: dış kovan Φ27,8 mm, 75 g.** 2212 (Φ28 mm) bu yük için küçük kalıyor. Gerçekçi çap **28 mm**, montaj payıyla en çok 32 mm. Eski 50 mm üst sınırı gereksiz kötümserdi |
| N122 | ✅ **`C_D0` aralığı daraldı: 0,0131 – 0,0210.** Varsayılan 0,0248 artık aralığın **tamamının** üstünde. Göbekler kaportalanırsa (uç iskeletleri zaten kaportalanacak, aynı imalat adımı) terim 0,0007'ye iner — **açık tasarım kararı** |
| N123 | ✅ **Hacim kapanışı sınandı** (`aero/volume.py`). Hafif hat brüt 185 L, kullanılabilir ~102 L, yüksüz dolum 21 L → **yüke 81 L kalıyor.** 13 kg için gereken yoğunluk yalnız **0,16 kg/L**; su, elektronik, hatta köpük kargo sığıyor. **Konfigürasyon hacim değil KÜTLE sınırlı** |
| N124 | 🎯 **Görev seçimi aerodinamiği etkilemez, yapıyı etkiler.** Dış kalıp çizgisi ok açısı yasalarıyla sabit; CFD yalnız ona bağlı, yani gorev seçilmeden yapılabilir. Yük yolları, yerel takviye ve ağırlık merkezi ise göreve bağlı — o da 8.2'nin "en çok yanılma ihtimali olan yer" dediği kütle bütçesi. **Sonuç: CFD şimdi yapılabilir, yapısal kütle bütçesi görev seçilmeden kovalanmamalı** |
| N125 | ✅ **Göbek kaportası: KARAR ERTELENDİ, gerekçeli.** Tasarımcının sezgisi (motor göbekleri zaten pürüzsüz) kısmen doğru: pervane somunu doğal spinner etkisi yapıyor ve koaksiyel çiftte arkadaki motor öndekinin izinde. Ama hobi sınıfı outrunner aerodinamik değil — düz üst yüzey, soğutma delikleri, vida başları. Kullanılan **C_D = 0,6** zaten çıplak silindir (0,9) ile kaportalı (0,2) arasında orta değer. **Kaportalamanın kazancı `C_D0`'da yalnız 0,0010**, L/D'de %0,5'ten az → sonucu değiştirmiyor, bu yüzden karar harcamaya değmez. Gerçek araç yapıldığında ölçülür |
| N126 | 🎯 **CFD mi panel mi — CFD (RANS) seçildi, ama "sonra".** Panel yöntemi zayıf halkayı çözmez: bizim sorunumuz **kalın gövdede potansiyel akış varsayımının zorlanması**, panel de aynı sınıf varsayımı taşıyor. Cevabı veren RANS'tır. **Ama doğrulanmamış bir CFD, beyan edilmiş bir varsayımdan DAHA BÜYÜK bir açıktır** — N120'de `C_D0` için reddettiğimiz takasın aynısı. Bu yüzden RANS ancak şu disiplinle yapılır: ağ bağımsızlığı, y+ denetimi, türbülans modeli duyarlılığı ve **yayımlanmış veriyle doğrulama vakası, kendi geometrimize dokunmadan önce** (VLM'de dikdörtgen kanatla yaptığımızın aynısı — orada e > 1 böyle yakalanmıştı) |
| N127 | **Sonuç: RANS v2 yaması değil, İKİNCİ MAKALEnin içeriğidir.** v2, elimizdeki sınırlanmış varsayımlarla çıkabilir — bu zaten gerçek bir güçlendirme. CFD, 8.12'nin söz verdiği takip çalışması olur. Ortamda OpenFOAM (apt), gmsh, meshio ve pyvista kurulabiliyor; 29 GB boş alan var, yani yapılabilirlik doğrulandı |
| N128 | ✅ **v2 makaleye işlendi.** Yeni **Bölüm 6.6 — "Independent checks on the two assumed coefficients"**: girdap-kafes çözümü (e ve CL_α), `C_D0` bileşen kurulumu, hacim kapanışı. Anahtar cümle: *"They remain assumed… it asks a narrower question: are the assumed values inside the range that a calculation gives, and on which side?"* Hesap varsayımı değiştirmiyor, sınırlıyor — N120'nin makaledeki karşılığı |
| N129 | ✅ **Bölüm 8 güncellendi.** 8.4 artık iki katsayının sınırlandığını *ve* kurulumun zayıf halkasını (2B şerit + %25 kalın merkez gövde) söylüyor. 8.6'ya eğim duyarlılığı eklendi. 8.12'nin 1. maddesi "bileşen kurulumu"ndan **"merkez gövdenin üç boyutlu çözümü"**ne çevrildi — çünkü birincisi artık yapıldı |
| N130 | ✅ **Özet 349'dan 279 kelimeye indirildi** ve son cümlesi eklendi: katsayılar hesapla *değiştirilmedi*, hesapla *sınırlandı*. Kaynakçaya [15] AeroSandbox ve [16] NeuralFoil girdi. Çıkar beyanına başvuru numarası (2026/014570), veri beyanına depo adresi eklendi |
