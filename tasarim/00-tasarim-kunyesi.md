...
| N118 | **Düzeltmenin yayıldığı yerler:** hafif hat L/D 12,7 → **12,0**, menzil 1 695 → **1 598 km**, dayanım 15,7 → **14,8 h**; ağır hat 14,0 → **13,6**, 1 868 → **1 814 km**, 13,0 → **12,6 h**. L/D düşünce seyir gücü yükseliyor: hafif 1,6 → **1,7 kW** elektrik ve 1,8 → **1,9 kW** motor mili; ağır 38,1 → **39,2 kW** ve 44,5 → **45,8 kW**. Motor dereceleri (2,6 / 54,3 kW) hâlâ yeterli. Şekil 12'nin işaretçileri ve `dogrula.py` de güncellendi |
| N119 | ⚠️ **Depo kaynağı artık yayımlanmış v1'den ileride.** Zenodo'daki v1 değişmez; düzeltme v2 ile gidecek. README'ye uyarı kondu ki depoya giren biri PDF ile kaynak arasındaki farkı hata sanmasın. `dogrula.py`'ye yeni bir kontrol eklendi: L/D'nin gerçekten seyir noktası polarından geldiğini sınıyor — 40 kontrol, sıfır sapma |
...
| N123 | ✅ **Hacim kapanışı sınandı** (`aero/hacim.py`). Hafif hat brüt 185 L, kullanılabilir ~102 L, yüksüz dolum 21 L → **yüke 81 L kalıyor.** 13 kg için gereken yoğunluk yalnız **0,16 kg/L**; su, elektronik, hatta köpük kargo sığıyor. **Konfigürasyon hacim değil KÜTLE sınırlı** |
